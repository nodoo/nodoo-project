...
genspecs/general-specifications_e_-section-3.pdf?sfvrsn=18, section 3.4.2,
        ...
        # Check if alternative separator is needed
        if (barcode_string.find(separator) == -1 and
            self.env.user.gs1_alternative_separator):
            # Use alternative separator
            separator = self.env.user.gs1_alternative_separator

        # Specific separator string for regular expressions
        ...
        FIXED_LENGTH = r'(?P<value>.{%d}' + re_separator + r'?)'
        ...
        VARIABLE_LENGTH = \
            r'(?P<value>[^' + re_separator + r']{%d,%d}' + re_separator + r'?)'
